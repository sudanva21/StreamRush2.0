...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, isMobile = false }) => {
  ...
  return (
    <aside className={`fixed left-0 top-16 bg-youtube-dark/95 backdrop-blur-md border-r border-youtube-gray/50 overflow-y-auto transition-all duration-300 ease-in-out z-40 ${
      isMobile 
        ? isOpen 
          ? 'w-72 h-full translate-x-0' 
          : 'w-72 h-full -translate-x-full'
        : isOpen 
          ? 'w-64 h-full' 
          : 'w-16 h-full'
    }`}>
      <div className="p-3">
        ...
        {/* Footer */}
        {(isOpen || isMobile) && (
          <div className="px-3 py-4 pb-safe-bottom text-xs text-youtube-lightgray border-t border-youtube-gray/50 mt-auto">
            <p className="mb-2">
