...
import { Video, Comment, Playlist, History, SearchFilters } from '../types';
...
interface VideoContextType {
  ...
  getWatchHistory: () => Promise<History[]>;
...
export const VideoProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addToWatchHistory = async (videoId: string) => {
    ...
    try {
      ...
      if (!existingHistory.empty) {
        ...
        const existingDoc = existingHistory.docs[0];
        await updateDoc(doc(db, 'history', existingDoc.id), {
  ...
  const getWatchHistory = async (): Promise<History[]> => {
    if (!currentUser) return [];
    ...
    try {
      ...
      // Fallback query without deleted filter if the query fails
      try {
  ...
  const removeFromWatchHistory = async (historyId: string) => {
    ...
    try {
      await updateDoc(doc(db, 'history', historyId), {
        // Firestore doesn't have real delete, so we mark as deleted
        deleted: true
      });
  ...
  const addToPlaylist = async (playlistId: string, videoId: string) => {
    ...
    try {
      ...
      if (playlistData.userId !== currentUser.uid) {
        toast.error('You can only add to your own playlists');
        return;
      }

      // Check if video is already in playlist
      const existingVideos = playlistData.videos || [];
      const videoExists = existingVideos.some((v: any) => v.videoId === videoId);
  ...
  const removeFromPlaylist = async (playlistId: string, videoId: string) => {
    if (!currentUser) {
      toast.error('Please login to modify playlists');
      return;
    }
    ...
    try {
      ...
      // Check if user owns this playlist
      if (playlistData.userId !== currentUser.uid) {
        toast.error('You can only modify your own playlists');
        return;
      }

      // Find and remove the video
      const videos = playlistData.videos || [];
      const updatedVideos = videos.filter((v: any) => v.videoId !== videoId);
  ...
  const getUserPlaylists = async (): Promise<Playlist[]> => {
    ...
    try {
      ...
      return querySnapshot.docs.map(doc => ({
        ...
        ...doc.data(),
        createdAt: doc.data().createdAt?.toDate(),
        videos: doc.data().videos?.map((video: any) => ({
          ...video,
          addedAt: video.addedAt?.toDate()
        })) || []
      })) as Playlist[];
    } catch (error) {
      console.error('Error fetching user playlists:', error);
      // Fallback query without deleted filter
      try {
        const fallbackQuery = query(
        ...
        console.error('Fallback playlist query failed:', fallbackError);
        return [];
      }
    }
  };
