  ...
  const loadPlaylist = async () => {
    ...
    try {
      ...
      if (playlistDoc.exists()) {
        ...
        // Load videos in playlist using new schema
        if (playlistData.videos && playlistData.videos.length > 0) {
          const videoIds = playlistData.videos.map((v: any) => v.videoId);
          ...
          // Maintain playlist order using new schema
          const orderedVideos = playlistData.videos
            .map((playlistVideo: any) => videosData.find(video => video.id === playlistVideo.videoId))
            .filter((video): video is Video => video !== undefined);

          setVideos(orderedVideos);
        } else if (playlistData.videoIds && playlistData.videoIds.length > 0) {
          ...
          const orderedVideos = playlistData.videoIds
            .map((videoId: string) => videosData.find(video => video.id === videoId))
            .filter((video): video is Video => video !== undefined);
        ...
        <div className="lg:w-1/3">
          <div className="bg-gradient-to-b from-red-600/20 to-purple-600/20 rounded-xl p-6 border border-red-500/20">
            ...
            <div className="text-sm text-youtube-lightgray mb-4">
              <p>{playlist.videos?.length || playlist.videoIds?.length || 0} videos</p>
              {videos.length > 0 && (
                <p>{formatTotalDuration(calculateTotalDuration())} total</p>
              )}
              <p>Created {formatDistanceToNow(playlist.createdAt, { addSuffix: true })}</p>
            </div>
